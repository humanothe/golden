...
import { LogOut, User, Shield, CreditCard, Settings, Sun, Moon } from 'lucide-react';
    ...
    <Layout>
      <div className="p-6 space-y-8">
        ...
        <div className="space-y-4">
          <button 
            onClick={toggleTheme}
            className="w-full bg-gold-400/10 border-2 border-gold-400/30 p-6 rounded-2xl flex items-center justify-between group hover:border-gold-400 transition-all"
          >
            <div className="flex items-center gap-4">
              {theme === 'dark' ? <Moon size={20} className="text-gold-400" /> : <Sun size={20} className="text-gold-400" />}
              <p className="text-xs font-black uppercase tracking-widest">Modo {theme === 'dark' ? 'Oscuro' : 'Claro'}</p>
            </div>
            <div className="w-10 h-5 bg-black/20 dark:bg-white/20 rounded-full relative">
              <div className={`absolute top-1 w-3 h-3 rounded-full bg-gold-400 transition-all ${theme === 'dark' ? 'left-6' : 'left-1'}`} />
            </div>
          </button>
