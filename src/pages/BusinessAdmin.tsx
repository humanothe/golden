...
import { 
    ...
    ShieldCheck, Copy, Check, LayoutGrid, TrendingUp, TrendingDown, DollarSign
} from 'lucide-react';
import { 
...
export const BusinessAdmin: React.FC = () => {
  ...
  const chartData = useMemo(() => {
    ...
    sortedEntries.forEach(entry => {
      const date = new Date(entry.created_at).toLocaleDateString('es-ES', { day: '2-digit', month: 'short' });
      if (!dailyData[date]) dailyData[date] = { income: 0, expense: 0 };
      
      const monto = Number(entry.monto);
      if (monto > 0) dailyData[date].income += monto;
      else dailyData[date].expense += Math.abs(monto);
    });
  ...
  const handleOpenModal = (index: number, existing?: any) => {
    ...
    if (!emailHijo) return showToast("Terminal no disponible.", 'error');
  ...
  const handleSaveStaff = async () => {
    ...
    if (!newStaff.name || newStaff.pin.length < 4) {
      return showToast("Completa los datos correctamente.", 'error');
    }
    setLoading(true);
    try {
      if (selectedTerminal.id) {
        ...
        showToast("NUEVA TERMINAL ACTIVADA");
      }
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#0c0c0c] flex flex-col w-full h-full overflow-hidden font-sans text-left transition-all duration-700">
      {/* TOAST SYSTEM */}
      {toast && (
        <div className={`fixed top-24 left-1/2 -translate-x-1/2 z-[10000] px-8 py-4 rounded-full backdrop-blur-3xl border shadow-2xl animate-enter-screen flex items-center gap-4 ${toast.type === 'success' ? 'bg-green-500/10 border-green-500/20 text-green-500' : 'bg-red-500/10 border-red-500/20 text-red-500'}`}>
              ...
              {activeTab === 'finance' && (
                <div className="space-y-10 animate-fade-in">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
                      <div className="p-8 bg-white/[0.03] border border-white/5 rounded-3xl shadow-2xl backdrop-blur-md relative overflow-hidden group">
                          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform">
                            <TrendingUp size={40} className="text-green-500" />
                          </div>
                          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-500 mb-6">INGRESOS_TOTALES</p>
                          <h3 className="text-4xl font-mono font-black text-white tracking-tighter mb-2">RD$ {financialStats.totalIn.toLocaleString()}</h3>
                          <div className="flex items-center gap-2 text-green-500">
                            <TrendingUp size={12} />
                            <span className="text-[9px] font-bold uppercase tracking-widest">Flujo Positivo</span>
                          </div>
                      </div>
                      ...
                      <div className="p-8 bg-white/[0.03] border border-white/5 rounded-3xl shadow-2xl backdrop-blur-md relative overflow-hidden group">
                          ...
                          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-500 mb-6">EGRESOS_TOTALES</p>
                          <h3 className="text-4xl font-mono font-black text-white tracking-tighter mb-2">RD$ {financialStats.totalOut.toLocaleString()}</h3>
                          <div className="flex items-center gap-2 text-red-500">
                            <TrendingDown size={12} />
                            <span className="text-[9px] font-bold uppercase tracking-widest">Retiros / Comisiones</span>
                          </div>
                      </div>

                      <div className="p-8 bg-gold-400/[0.03] border border-gold-400/20 rounded-3xl shadow-2xl backdrop-blur-md relative overflow-hidden group">
                          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform">
                            <DollarSign size={40} className="text-gold-400" />
                          </div>
                          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gold-400/60 mb-6">BALANCE_NETO</p>
                          <h3 className="text-4xl font-mono font-black text-gold-400 tracking-tighter mb-2">RD$ {(financialStats.totalIn - financialStats.totalOut).toLocaleString()}</h3>
                          <div className="flex items-center gap-2 text-gold-400">
                            <Activity size={12} />
                            <span className="text-[9px] font-bold uppercase tracking-widest">Rendimiento Nodo</span>
                          </div>
                      </div>
                  </div>

                  <div className="p-10 bg-white/[0.02] border border-white/5 rounded-[2.5rem] shadow-2xl">
                    <div className="flex items-center justify-between mb-10">
                      <div>
                        <h4 className="text-sm font-black uppercase tracking-[0.4em] text-white mb-1">Análisis de Rendimiento</h4>
                        <p className="text-[9px] text-gray-600 font-bold uppercase tracking-widest">Últimos 7 ciclos de operación</p>
                      </div>
                      <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2">
                          <div className="w-2 h-2 rounded-full bg-gold-400"></div>
                          <span className="text-[8px] font-black text-gray-500 uppercase tracking-widest">Ingresos</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="w-2 h-2 rounded-full bg-white/20"></div>
                          <span className="text-[8px] font-black text-gray-500 uppercase tracking-widest">Egresos</span>
                        </div>
                      </div>
                    </div>
