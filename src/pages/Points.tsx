...
export const Points: React.FC = () => {
  ...
  const handleRedeem = async (cost: number, reward: string) => {
    if (!user?.email || currentPoints < cost) return;
    
    try {
      const { error } = await supabase.from('mi_saldo').insert([{
        socio_email: user.email,
        monto: -cost,
        concepto: `CANJE: ${reward.toUpperCase()}`,
      ...
      alert(`¡CANJE EXITOSO! Has obtenido: ${reward}`);
    } catch (e: any) {
    ...
    <div className="min-h-screen relative overflow-hidden transition-colors duration-500">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 py-8 pb-32">
        <div className="flex items-center gap-4 mb-8">
            ...
            <h1 className="font-heading text-2xl font-light text-gray-900 dark:text-white">Golden Puntos</h1>
        </div>
        ...
        <div className="flex justify-center mb-10">
            <div className="relative w-48 h-48 flex items-center justify-center">
                ...
                <div className="text-center">
                    <span className="block text-4xl font-heading font-bold text-gray-900 dark:text-white">{currentPoints}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className={`glass-panel p-5 rounded-3xl bg-white/60 dark:bg-white/5 border border-gray-200 dark:border-white/5 flex gap-4 items-center ${currentPoints < 1000 ? 'opacity-60 grayscale' : ''}`}>
                <div className="w-12 h-12 rounded-2xl bg-purple-500/10 flex items-center justify-center text-purple-500">
                    <Gift size={24} />
                </div>
                <div>
                    <h4 className="font-bold text-gray-900 dark:text-white text-sm">Cupón $10 OFF</h4>
                    <p className="text-xs text-gold-500 font-bold mt-1">1000 Puntos</p>
                </div>
                {currentPoints >= 1000 ? (
                    <button 
                      onClick={() => handleRedeem(1000, "Cupón $10 OFF")}
                      className="ml-auto px-4 py-2 bg-gray-900 dark:bg-white text-white dark:text-black text-[10px] font-bold uppercase rounded-full tracking-wider hover:scale-105 transition-transform"
                    >
                      Canjear
                    </button>
                ) : (
            ...
            {/* Locked Reward */}
            <div className={`glass-panel p-5 rounded-3xl bg-white/40 dark:bg-white/5 border border-gray-200 dark:border-white/5 flex gap-4 items-center ${currentPoints < 5000 ? 'opacity-60 grayscale' : ''}`}>
                <div className="w-12 h-12 rounded-2xl bg-gray-200 dark:bg-white/10 flex items-center justify-center text-gray-400">
                    <Lock size={24} />
                </div>
                <div>
                    <h4 className="font-bold text-gray-900 dark:text-white text-sm">Cena VIP (2 pers)</h4>
