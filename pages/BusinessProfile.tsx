    ...
    <div className="min-h-screen bg-light-50 dark:bg-dark-950 pb-28">
      ...
      <div className="relative h-64 md:h-80 w-full overflow-hidden">
        ...
        <div className="absolute top-6 left-6 z-20">
            <button onClick={() => navigate('/partners')} className="p-3 bg-black/20 dark:bg-white/20 backdrop-blur-md rounded-full text-black dark:text-white hover:bg-white/30 transition-colors">
                <ArrowLeft size={20} />
            </button>
        </div>
      </div>
      ...
      <div className="relative z-20 -mt-10 px-4 max-w-5xl mx-auto">
        <div className="glass-panel p-6 rounded-[2rem] bg-white/90 dark:bg-dark-900/95 backdrop-blur-xl border border-gray-200 dark:border-black/10 dark:border-white/10 shadow-xl flex flex-col md:flex-row gap-6 items-start">
            ...
            <div className="flex-1">
                <h1 className="text-2xl font-heading font-bold text-gray-900 dark:text-black dark:text-white mb-2">{business.name}</h1>
                <div className="flex flex-wrap gap-4 text-xs text-gray-500 dark:text-gray-400 mb-4">
                    ...
                    <span className="px-2 py-0.5 rounded-full bg-gray-100 dark:bg-black/10 dark:bg-white/10 uppercase tracking-wider text-[10px] font-bold">{business.category}</span>
                </div>
      ...
      <div className="max-w-5xl mx-auto px-4 mt-8">
        <div className="flex gap-8 border-b border-gray-200 dark:border-black/10 dark:border-white/10 mb-6">
            <button 
        ...
        {activeTab === 'menu' ? (
                    ...
                    {categories.map(cat => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-full text-xs font-bold uppercase whitespace-nowrap transition-all ${
                                activeCategory === cat 
                                ? 'bg-white dark:bg-black dark:bg-black dark:bg-white text-white dark:text-black dark:text-white dark:text-black' 
                                : 'bg-gray-100 dark:bg-black/5 dark:bg-white/5 text-gray-500 hover:bg-gray-200 dark:hover:bg-black/10 dark:bg-white/10'
                            }`}
                        >
                            {cat === 'all' ? 'Todos' : cat}
                        </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {filteredProducts.map(product => (
                        <div 
                            ...
                            className="glass-panel p-4 rounded-2xl bg-white/60 dark:bg-black/5 dark:bg-white/5 border border-gray-200 dark:border-black/5 dark:border-white/5 hover:border-gold-400/30 flex gap-4 cursor-pointer group transition-all"
                        >
                            ...
                            <div className="flex flex-col flex-1 justify-between">
                                <div>
                                    <h3 className="font-bold text-gray-900 dark:text-black dark:text-white text-sm line-clamp-1">{product.name}</h3>
                                ...
                                <div className="flex justify-between items-end mt-2">
                                    ...
                                    <div className="p-2 bg-white dark:bg-black dark:bg-white rounded-full text-black dark:text-white dark:text-black">
                                        <Plus size={14} />
                                    </div>
                                </div>
                            </div>
            ...
            /* Info Tab */
            <div className="glass-panel p-6 rounded-2xl bg-white/60 dark:bg-black/5 dark:bg-white/5 space-y-6">
                <div>
                    <h3 className="font-bold text-gray-900 dark:text-black dark:text-white mb-2 flex items-center gap-2"><MapPin size={16} /> Ubicación</h3>
                    <p className="text-sm text-gray-500 font-light">{business.address || 'Ubicación no disponible'}</p>
                </div>
                <div>
                    <h3 className="font-bold text-gray-900 dark:text-black dark:text-white mb-2 flex items-center gap-2"><Clock size={16} /> Horarios</h3>
                    <p className="text-sm text-gray-500 font-light">Lunes - Domingo: 10:00 AM - 10:00 PM</p>
                </div>
                <div>
                    <h3 className="font-bold text-gray-900 dark:text-black dark:text-white mb-2 flex items-center gap-2"><Info size={16} /> Detalles</h3>
