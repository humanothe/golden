...
export const Market: React.FC = () => {
  ...
  const handleFreezePrice = async () => {
    ...
    try {
        const activeFreezes = await api.data.getActiveFreezesCount(user.email);
        const currentPlan = MEMBERSHIP_PLANS.find(p => p.name.toLowerCase() === user.membership_tier.toLowerCase());
        const limit = currentPlan?.limit || 0;

        if (activeFreezes >= limit) {
            setErrorFeedback(`LÍMITE ALCANZADO: TU PLAN ${user.membership_tier.toUpperCase()} SOLO PERMITE ${limit} ACTIVOS CONGELADOS SIMULTÁNEAMENTE.`);
            return;
        }
    } catch (e) {
        console.error("LIMIT_CHECK_ERROR", e);
    }
