...
export const DeliveryDashboard: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 lg:px-12 py-6 pb-28 space-y-6 relative min-h-screen">
        ...
        {/* HEADER */}
        <div className="flex justify-between items-center py-4 border-b border-gray-200 dark:border-black/10 dark:border-white/10">
            <div>
                <h1 className="text-2xl font-heading font-bold dark:text-black dark:text-white flex items-center gap-2">
            ...
            <div className="flex gap-4">
                <button onClick={fetchDeliveryData} className="p-3 bg-gray-100 dark:bg-black/10 dark:bg-white/10 rounded-full text-gray-500 hover:rotate-180 transition-transform">
                    <RefreshCw size={20} />
                </button>
                <button 
                    onClick={() => setIsOnline(!isOnline)}
                    className={`p-3 rounded-full border transition-all ${isOnline ? 'bg-green-500 text-black dark:text-white border-green-500 shadow-lg shadow-green-500/30' : 'bg-transparent text-gray-400 border-gray-300 dark:border-white/20'}`}
                >
                    <Power size={20} />
                </button>
            </div>
        ...
        <div className="grid grid-cols-3 gap-4">
            <div className="glass-panel p-4 rounded-xl bg-white dark:bg-black dark:bg-black dark:bg-white text-white dark:text-black dark:text-white dark:text-black col-span-1">
                <p className="text-[10px] font-bold uppercase opacity-70">Hoy</p>
                <h2 className="text-xl font-heading font-bold">$1,200</h2>
            </div>
            <div className="glass-panel p-4 rounded-xl bg-white dark:bg-black/5 dark:bg-white/5 col-span-2">
                <p className="text-[10px] text-gray-500 uppercase font-bold">Estado Actual</p>
                <p className="text-sm font-bold dark:text-black dark:text-white flex items-center gap-2">
        ...
        {activeOrder ? (
            <div className="mt-4 animate-fade-in">
                ...
                <div className="glass-panel p-6 rounded-[2rem] bg-gradient-to-br from-gold-400/10 to-transparent border-gold-400/30 border shadow-xl relative overflow-hidden">
                    ...
                    <div className="flex items-start gap-4 mb-6">
                        <div className="w-12 h-12 bg-black/10 dark:bg-white/10 rounded-full flex items-center justify-center border border-white/20">
                            <Clock className="text-gold-500" />
                        </div>
                        <div>
                            <p className="text-xs text-gray-500 uppercase font-bold">Pedido #{activeOrder.id.substring(0,6)}</p>
                            <h3 className="text-lg font-bold dark:text-black dark:text-white">Pago: ${activeOrder.total.toLocaleString()}</h3>
                        </div>
                    </div>

                    {/* Timeline / Steps */}
                    <div className="relative pl-4 space-y-6 mb-8">
                        <div className="absolute left-[7px] top-2 bottom-2 w-0.5 bg-gray-200 dark:bg-black/10 dark:bg-white/10"></div>
                        ...
                        <div className={`relative flex items-start gap-4 ${activeOrder.status === 'picked_up' ? 'opacity-50' : 'opacity-100'}`}>
                            ...
                            <div>
                                ...
                                <p className="font-bold dark:text-black dark:text-white">{activeOrder.business_name}</p>
                        ...
                        <div className={`relative flex items-start gap-4 ${activeOrder.status === 'picked_up' ? 'opacity-100' : 'opacity-50'}`}>
                            ...
                            <div>
                                ...
                                <p className="font-bold dark:text-black dark:text-white">{activeOrder.customer_name}</p>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                         <button className="py-3 bg-gray-100 dark:bg-black/10 dark:bg-white/10 rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 hover:bg-gray-200 dark:hover:bg-black/20 dark:bg-white/20">
                         ...
                         {activeOrder.status === 'ready' && (
                             <button 
                                ...
                                className="col-span-2 py-4 bg-white dark:bg-black dark:bg-black dark:bg-white text-white dark:text-black dark:text-white dark:text-black rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform"
                             >
                         ...
                         {activeOrder.status === 'picked_up' && (
                             <button 
                                ...
                                className="col-span-2 py-4 bg-green-600 text-black dark:text-white rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform shadow-lg shadow-green-500/20"
                             >
            ...
            <div className="mt-8">
                ...
                {!isOnline ? (
                    <div className="p-10 text-center glass-panel rounded-2xl border-dashed border-2 border-gray-200 dark:border-black/10 dark:border-white/10 opacity-60">
                        <Truck size={48} className="mx-auto mb-4 text-gray-400" />
                        <h3 className="font-bold text-lg dark:text-black dark:text-white">Estás desconectado</h3>
                        <p className="text-sm text-gray-500">Conéctate para ver el radar de pedidos.</p>
                    </div>
                ) : availableOrders.length === 0 ? (
                    <div className="p-10 text-center glass-panel rounded-2xl border border-gray-200 dark:border-black/10 dark:border-white/10">
                        <Loader2 size={32} className="mx-auto mb-4 text-gold-400 animate-spin" />
                        <h3 className="font-bold text-sm dark:text-black dark:text-white">Buscando pedidos cercanos...</h3>
                        <p className="text-xs text-gray-500 mt-2">Mantente atento, aparecerán aquí.</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {availableOrders.map(order => (
                            <div key={order.id} className="glass-panel p-5 rounded-2xl bg-white dark:bg-black/5 dark:bg-white/5 border border-gray-100 dark:border-black/5 dark:border-white/5 hover:border-gold-400 transition-all group">
                                <div className="flex justify-between items-start mb-3">
                                    <div>
                                        ...
                                        <h3 className="font-bold dark:text-black dark:text-white">{order.business_name}</h3>
                                        <p className="text-xs text-gray-500">{order.business_address}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-bold text-lg dark:text-black dark:text-white">${(order.total * 0.15).toFixed(0)}</p>
                                        <p className="text-[10px] text-gray-400 uppercase">Ganancia Est.</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2 text-xs text-gray-500 mb-4 border-t border-gray-100 dark:border-black/5 dark:border-white/5 pt-2">
