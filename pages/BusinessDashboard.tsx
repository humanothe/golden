...
export const BusinessDashboard: React.FC = () => {
  ...
  if (loading) return <div className="min-h-screen bg-white dark:bg-black flex items-center justify-center"><div className="w-10 h-10 border-2 border-gold-400 border-t-transparent rounded-full animate-spin"></div></div>;

  return (
    <div className="min-h-screen bg-white dark:bg-[#050505] text-black dark:text-white p-6 lg:p-12 pb-32 font-sans animate-fade-in">
      
      <header className="pt-[env(safe-area-inset-top)] px-6 md:px-12 border-b border-white/10 sticky top-0 z-40 bg-black -mx-6 lg:-mx-12 mb-12">
        <div className="max-w-[1400px] mx-auto pb-6 pt-6 flex justify-between items-end">
            <div>
              ...
              <h1 className="text-4xl md:text-6xl font-heading font-bold uppercase tracking-tighter leading-none text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        ...
        <div className="glass-panel p-10 rounded-[2.5rem] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 flex flex-col justify-between">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">Pedidos Recibidos</p>
            <h3 className="text-5xl font-heading font-bold text-black dark:text-white">{orders.length}</h3>
          </div>
          <button className="w-full py-4 border border-black/10 dark:border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest mt-8 hover:bg-black dark:hover:bg-white hover:text-black transition-all">Ver Historial</button>
        </div>

        <div className="glass-panel p-10 rounded-[2.5rem] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 flex flex-col justify-between">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">Catálogo Activo</p>
            <h3 className="text-5xl font-heading font-bold text-black dark:text-white">8</h3>
          </div>
          <button className="w-full py-4 bg-black dark:bg-white text-white dark:text-black rounded-2xl text-[10px] font-black uppercase tracking-widest mt-8 flex items-center justify-center gap-2">
            <PlusCircle size={14} /> Gestionar Menú
          </button>
        </div>
      </div>

      <div className="p-10 rounded-[3rem] bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 relative overflow-hidden">
