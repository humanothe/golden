...
export const HistoryPage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: bizData } = await supabase
        .from('transactions')
        .select('status, business_name, created_at, id, gross_amount, net_savings')
  ...
  const getStatusInfo = (status: string) => {
    ...
    if (s === 'aprobado' || s === 'approved' || s === 'confirmado') {
  ...
  return (
    <div className="min-h-screen bg-transparent text-white p-6 md:p-12 pb-40 font-sans text-left overflow-x-hidden selection:bg-gold-400 selection:text-black relative">
      ...
      {/* ATMÓSFERA LUMÍNICA - HISTORY (Invertida: Oro Top-Right, Blanco Bottom-Left) */}
      <div className="fixed inset-0 z-0 pointer-events-none">
          <div className="absolute top-0 right-0 w-[70vw] h-[50vh] opacity-[0.15]"
               style={{ background: 'radial-gradient(circle at 100% 0%, #D4AF37 0%, transparent 60%)', filter: 'blur(100px)' }}></div>
          <div className="absolute bottom-0 left-0 w-[80vw] h-[50vh] opacity-[0.3]"
               style={{ background: 'radial-gradient(circle at 0% 100%, white 0%, transparent 70%)', filter: 'blur(80px)' }}></div>
          <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-5 pointer-events-none"></div>
      </div>

      <div className="relative z-10">
          <header className="max-w-xl mx-auto flex items-center justify-between mb-12">
              <div className="flex items-center gap-6">
                <button 
                  onClick={() => navigate('/profile')} 
                  className="p-4 bg-white/5 border border-white/10 rounded-none hover:bg-gold-400 hover:text-black transition-all active:scale-90"
                >
                  <ArrowLeft size={24} />
                </button>
                <div>
                  <p className="text-[10px] text-gold-400 font-black uppercase tracking-[0.5em] mb-2">Libro Mayor Maestro</p>
                  <h1 className="text-3xl font-heading font-black uppercase tracking-tighter text-white leading-none">
          ...
          <div className="max-w-xl mx-auto space-y-8">
            <div className="p-8 bg-white/[0.02] border border-white/5 rounded-none shadow-2xl relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-32 h-32 bg-gold-400/5 blur-[50px] -mr-16 -mt-16"></div>
              <p className="text-[8px] text-gray-600 font-black uppercase tracking-[0.6em] mb-4">SALDO_TOTAL_LIQUIDADO</p>
              <div className="flex items-baseline gap-3">
                <h2 className="text-4xl md:text-5xl font-mono font-black tracking-tighter text-white leading-none">
                  {totalBalance.toLocaleString()}
                </h2>
                <span className="text-lg font-heading font-bold text-gold-400 opacity-30">GP</span>
              </div>
            </div>
            ...
            <section className="space-y-4">
              {loading ? (
                <div className="py-24 flex flex-col items-center gap-6">
                  ...
                  <p className="text-[9px] font-black uppercase tracking-widest text-gray-700">Consultando Nodo Maestro...</p>
                </div>
              ) : (
                <>
                  {activeTab === 'vault' ? (
                    <div className="space-y-2">
                      {vaultHistory.length === 0 ? (
                        ...
                        return (
                          <div key={idx} className="p-5 bg-[#080808] border border-white/5 flex items-center justify-between hover:border-white/10 transition-all">
                            <div className="flex items-start gap-4">
                               <div className={`p-2.5 rounded-none ${isPositive ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'} mt-0.5`}>
                               ...
                               <div>
                                 ...
                                 <p className="text-[8px] text-gray-600 font-bold uppercase">{new Date(move.created_at).toLocaleDateString()}</p>
                               </div>
                            </div>
                            <div className="text-right">
                               ...
                               <p className="text-[7px] text-gray-700 font-black uppercase tracking-widest">PUNTOS_GP</p>
                            </div>
                          </div>
                        );
                      })}
                          ...
                          <div key={item.id} className="p-6 bg-white/[0.01] border border-white/5 hover:bg-white/[0.02] transition-all">
                            <div className="flex justify-between items-start mb-4">
                               <div className="flex items-start gap-4">
                                  ...
                                  <div>
                                     ...
                                     <h4 className="text-sm font-heading font-black text-white uppercase leading-none mb-2">{item.business_name || 'ESTABLECIMIENTO'}</h4>
                            ...
                            <div className="flex justify-between items-end pt-4 border-t border-white/5">
                                <div>
                                    ...
                                    <p className="text-[10px] font-mono text-white/40">RD$ {Number(item.gross_amount || 0).toLocaleString()}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-[8px] text-gold-400 font-black uppercase mb-1">Ahorro Aplicado</p>
                                    <p className="text-xl font-mono font-black text-white leading-none">RD$ {Math.abs(Number(item.net_savings || 0)).toLocaleString()}</p>
                                </div>
                            </div>
                          </div>
                        );
